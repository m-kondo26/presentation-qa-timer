...
const translations = {
  ja: {
    ...
    chimeVolume: "チャイム音量",
    usageTitle: "使用上の注意",
    usageDescription: "最初に必ず「監視を開始」を押してください。押さない限り、別タブ表示や最小化時を含めチャイムは鳴りません。iPadでは音声の許可に最初のタップが必要です。対応するSafari・Chromeでは、チャイムの間だけほかのアプリの音声を抑えるようOSへ要求し、終了後に解除します（音楽が一時停止する場合があります）。Chromeでは次回以降のチャイムを音声時計へ先行予約し、別タブ表示中のタイマー遅延を避けます。別アプリ表示中や画面消灯中はOSがWebページ全体を停止することがあるため、確実に鳴らす場合はページを前面表示し、消音設定と本体音量を確認してください。",
